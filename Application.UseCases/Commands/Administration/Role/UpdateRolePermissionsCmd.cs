...
public record UpdateRolePermissionsCmd(IEnumerable<RolePermissionDTO> Permissions) : ITransactionalRequest<bool>;

public class UpdateRolePermissionsCmdHandler(
    IAppReadRepository appReadRepo,
    IAppCommandRepository appCommandRepo)
    : IRequestHandler<UpdateRolePermissionsCmd, bool>
{
    public async Task<bool> Handle(UpdateRolePermissionsCmd request, CancellationToken cancellationToken)
    {
        ...
        var roles = await appReadRepo.GetListAsync<RoleDEM>(r => roleIds.Contains(r.Id), true);
